...
| Questions requirements ("unclear", "what about", "edge case") | If `problem.md` exists: run `/clarify`. If not: run `/bmad-break`. |
...
| `/clarify` | Structured ambiguity scan | `problem.md` OR a project description as argument |
| `/ralph` | Full implementation — backlog to code | Backlog file, `backlog.md`, OR a text description |
...
|---------|-------------|----------|
| `/bmad-model` | Architecture + backlog design | `problem.md` |
| `/ux-spec` | UX design — flows, wireframes, interactions | `problem.md` OR feature description |
| `/analyze` | Cross-artifact consistency check (read-only) | `problem.md` + `architecture.md` + `backlog.md` |
| `/checklist` | Pre-implementation quality gate | `problem.md` + `architecture.md` + `backlog.md` |
| `/gsd-prep` | Codebase mapping + context packs for teammates | `backlog.md` + `architecture.md` |
| `/bmad-act` | Implement from backlog (delegates to Ralph) | `backlog.md` + `architecture.md` |
